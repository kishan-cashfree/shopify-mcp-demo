...
/**
 * The Cashfree lockup — symbol and wordmark — from the brand pack's
 * `Coloured logo/Light Background/cashfree logo full (color) on white.svg`.
 *
 * Three edits to that file, and nothing else:
 *
 *  - **The white plate is gone.** The asset opens with
 *    `<rect width="370" height="80" fill="white"/>`, and its dark-background
 *    twin differs by filling that same rect black. Shipping either verbatim
 *    paints a plate that suits exactly one of Claude's two themes. Without it
 *    there is one lockup, not a light one and a dark one.
 *  - **The wordmark is `currentColor`.** The brand pack fills the letterforms
 *    fixed black here and fixed white in the dark file. These marks sit inside
 *    lines whose colour belongs to the host — `text-secondary`, `text-tertiary`
 *    — and the widget cannot read the host's theme, so a fixed fill would be
 *    invisible on one of them. This is why the name used to be a text node in
 *    a system font; inheriting the colour is what makes the real letterforms
 *    usable. The symbol's gradients are left alone: teal-to-green and
 *    amber-to-pink read on white and on dark alike.
 *  - **The duplicate paths are dropped.** The export draws each symbol path
 *    twice — `paint0` is identical to `paint1`, and `paint2` carries the same
 *    `d` as `paint3` underneath it, fully overpainted. Two of the four are
 *    invisible; keeping them cost 766 characters of path data per render.
 *
 * Inline rather than an <img> or a data: URI. The widget is inlined into one
 * HTML document handed to the host, so a file would need either base64 paid
 * for in every resources/read or a remote origin declared in both CSP blocks —
 * the two blocks whose drift once blocked every product image on Claude. It
 * also could not take `currentColor`, which is the whole point above.
 *
